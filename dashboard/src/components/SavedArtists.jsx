...
// "Remove from saved" action. Clicking a card opens the same detail modal as
// the Leads tab.
    ...
    <div className="saved">
      ...
      {saved.length === 0 ? (
        ...
        <div className="saved-grid">
          {saved.map((item) => (
            <SavedItem key={item.id} item={item} onSelect={setSelected} />
          ))}
        </div>
      )}

      {selected && <ArtistDetailModal lead={selected} onClose={() => setSelected(null)} />}
    </div>
  );
}

function SavedItem({ item, onSelect }) {
  ...
  return (
    <div className="saved-item">
      <ArtistCard lead={item.lead} onSelect={onSelect} />
